...
class BinaryLogReg:
    ...
    def mu(self, X: np.ndarray, y: np.ndarray) -> np.ndarray:
        ...
        """
        self.mu_arr = 1.0/(1+np.exp(-y*((X@self.weight)+self.bias)))
        return self.mu_arr
    ...
    def loss(self, X: np.ndarray, y: np.ndarray) -> float:
        ...
        """
        self.mu(X,y)
        return np.log(1/self.mu_arr).mean()+ self._lambda * np.inner(self.weight,self.weight)
    ...
    def gradient_J_weight(self, X: np.ndarray, y: np.ndarray) -> np.ndarray:
        ...
        """
        self.mu(X,y) 
        a = ((X.T)*((self.mu_arr-1)*y)).mean(axis =1)
        b = 2* self._lambda * self.weight
        return a+b
    ...
    def misclassification_error(self, X: np.ndarray, y: np.ndarray) -> float:
        ...
        accuracy = ((predict == y).sum())/X.shape[0]
        return 1-accuracy
    ...
    def train(
        ...
        indexlist = list(range(self.n))
        for i in range(epochs):
            RNG.shuffle(indexlist)
            for ii in range(num_batches):
                indexs = indexlist[(ii)*batch_size:(ii+1)*batch_size]
                self.step(X_train[indexs,:],y_train[indexs],learning_rate)
            ...
            trainloss = result["train_losses"][-1]
            testloss = result["test_losses"][-1]
            # print(f"======= epoch : {i+1}, train loss : {trainloss}, test loss: {testloss}")
